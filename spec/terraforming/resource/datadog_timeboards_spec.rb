...
module Terraforming
  module Resource
    describe DatadogTimeboard do
      ...
      let(:get_all_timeboards_response) do
        [
          "200",
          [
            {
      ...
      before do
        allow(client).to receive(:get_all_timeboards).and_return(get_all_timeboards_response)
      end

      describe ".tf" do
        it "should generate tf" do
          expect(described_class.tf(client)).to eq <<-'EOS'
resource "datadog_timeboard" "High-Load-Average-hostname-namename" {
    name               = "High Load Average {{host.name}} {{name.name}}"
    type               = "metric alert"
    message            = <<EOT
@slack-infrastructure
...
resource "datadog_timeboard" "CriticalVPCAWS-Service-Status" {
    name               = "[Critical][VPC]AWS Service Status"
    type               = "service check"
    message            = <<EOT
@slack-engineering @slack-infrastructure
            ...
            "modules" => [
              {
                ...
                "outputs" => {},
                "resources" => {
                  "datadog_timeboard.High-Load-Average-hostname-namename" => {
                  ...
                  "datadog_timeboard.CriticalVPCAWS-Service-Status" => {
                    "type" => "datadog_timeboard",
                    "primary" => {
                      "id" => "789012",
                      "attributes" => {
